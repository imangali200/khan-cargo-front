...
async function getBranch() {
    loading.value = true
    try {
        const res = await $axios.get(`branch/${branchId}`, {
            headers: {
                'Authorization': `Bearer ${token.value}`
            }
        })
        currentBranch.value = res.data
        branchName.value = res.data.branchName
        selectAdmin.value = res.data.responsibleName
    } catch (error: any) {
        // console.error(error)
...
const updateBranch = async () => {
    ...
    try {
        await $axios.put(`branch/${branchId}`, {
...
<template>
    <div class="tw-py-6 animate-fadeIn">
        ...
        <!-- Header Card -->
        <div class="tw-bg-gradient-to-r tw-from-violet-500 tw-to-violet-600 tw-rounded-2xl tw-p-6 tw-mb-6 tw-shadow-lg tw-shadow-violet-500/20">
        ...
        <div v-if="loading" class="tw-bg-white/[0.03] tw-backdrop-blur-xl tw-border tw-border-white/10 tw-rounded-2xl tw-p-12 tw-text-center">
            <div class="tw-inline-flex tw-items-center tw-justify-center tw-w-16 tw-h-16 tw-bg-violet-500/20 tw-rounded-full tw-mb-4">
                <div class="tw-animate-spin tw-rounded-full tw-h-8 tw-w-8 tw-border-4 tw-border-violet-500 tw-border-t-transparent"></div>
            </div>
                ...
                <form class="tw-space-y-6" @submit.prevent="updateBranch">
                    ...
                    <div class="tw-space-y-2">
                        <label class="tw-flex tw-items-center tw-gap-2 tw-text-sm tw-font-semibold tw-text-white/80" for="nameBranch">
                            <svg class="tw-w-5 tw-h-5 tw-text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="tw-space-y-2">
                        ...
                        <div class="tw-relative">
                            ...
                            >
                                ...
                                <option 
                                    v-if="selectAdmin && !adminList.find(a => a.name + ' ' + a.surname === selectAdmin)"
                                    :value="selectAdmin"
                                    class="tw-bg-gray-800"
                                >
                                    {{ selectAdmin }} (текущий)
