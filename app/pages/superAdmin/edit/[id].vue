...
<template>
    <div class="tw-py-6 animate-fadeIn">
        ...
        <div class="tw-flex tw-items-center tw-gap-2 tw-flex-wrap tw-text-sm tw-mb-6">
            ...
            <router-link class="tw-text-cyan-400 hover:tw-text-cyan-300 tw-transition-colors"
                to="/superAdmin/users">Пользователи</router-link>
            <span class="tw-text-white/30">→</span>
        ...
        <div class="tw-flex tw-items-center tw-justify-between tw-mb-6 tw-pb-5 tw-border-b tw-border-white/10">
            ...
            <button @click="goToBack"
                class="tw-px-5 tw-py-2.5 tw-bg-red-500/20 tw-border tw-border-red-500/30 tw-rounded-xl tw-text-red-400 tw-font-medium hover:tw-bg-red-500/30 tw-transition-all">
                Назад
            </button>
        </div>
        ...
        <div v-if="loading" class="tw-text-center tw-py-12">
            <div
                class="tw-w-10 tw-h-10 tw-border-4 tw-border-cyan-500/20 tw-border-t-cyan-500 tw-rounded-full tw-animate-spin tw-mx-auto">
            </div>
        ...
        <form v-else @submit.prevent="updateUser"
            class="tw-bg-white/[0.03] tw-backdrop-blur-xl tw-border tw-border-white/10 tw-rounded-2xl tw-p-6 tw-flex tw-flex-col tw-gap-5">
            <!-- Код -->
            <div class="tw-flex tw-flex-col tw-gap-2">
                <label class="tw-text-sm tw-text-white/60" for="code">Код (для клиентов)</label>
                <input type="text" id="code" v-model="codeUser" placeholder="Например: KZ001"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white placeholder:tw-text-white/30 tw-outline-none focus:tw-border-cyan-500 tw-transition-all">
            </div>
            ...
            <div class="tw-flex tw-flex-col tw-gap-2">
                ...
                <input v-model="phoneNumber" type="text" id="phoneNumber" placeholder="8701 001 0101"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white placeholder:tw-text-white/30 tw-outline-none focus:tw-border-cyan-500 tw-transition-all">
            </div>
            ...
            <div class="tw-flex tw-flex-col tw-gap-2">
                ...
                <input id="name" type="text" v-model="name" placeholder="Ivan"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white placeholder:tw-text-white/30 tw-outline-none focus:tw-border-cyan-500 tw-transition-all">
            </div>
            ...
            <div class="tw-flex tw-flex-col tw-gap-2">
                ...
                <input type="text" id="surname" v-model="surname" placeholder="Ivanov"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white placeholder:tw-text-white/30 tw-outline-none focus:tw-border-cyan-500 tw-transition-all">
            </div>
            ...
            <div class="tw-flex tw-flex-col tw-gap-2 tw-w-full sm:tw-w-[280px]">
                ...
                <select id="role" v-model="role"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white tw-outline-none focus:tw-border-cyan-500 tw-transition-all tw-cursor-pointer">
            ...
            <div class="tw-flex tw-flex-col tw-gap-2 tw-w-full sm:tw-w-[320px]">
                ...
                <select id="branch" v-model="selectBranch"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white tw-outline-none focus:tw-border-cyan-500 tw-transition-all tw-cursor-pointer">
                    <option value="" class="tw-bg-[#1e293b]">Не выбран</option>
                    <option v-for="branch in branches" :key="branch.id" :value="branch.id" class="tw-bg-[#1e293b]">
                        {{ branch.name }}
                    </option>
                </select>
            </div>
            ...
            <div class="tw-flex tw-flex-col tw-gap-2">
                ...
                <input type="text" id="userPassword" v-model="password" placeholder="Введите новый пароль"
                    class="tw-h-12 tw-px-4 tw-bg-white/5 tw-border tw-border-white/10 tw-rounded-xl tw-text-white placeholder:tw-text-white/30 tw-outline-none focus:tw-border-cyan-500 tw-transition-all">
            </div>

            <!-- Error message -->
            <p v-if="errorMessage"
                class="tw-text-red-400 tw-text-sm tw-bg-red-500/10 tw-border tw-border-red-500/20 tw-rounded-xl tw-px-4 tw-py-3">
                {{ errorMessage }}</p>

            <!-- Submit -->
            <button type="submit" :disabled="submitting"
                class="tw-h-12 tw-w-full sm:tw-w-[220px] tw-bg-gradient-to-r tw-from-cyan-500 tw-to-cyan-600 tw-rounded-xl tw-text-white tw-font-semibold hover:tw-from-cyan-600 hover:tw-to-cyan-700 hover:tw-shadow-lg hover:tw-shadow-cyan-500/30 tw-transition-all disabled:tw-opacity-50 disabled:tw-cursor-not-allowed tw-flex tw-items-center tw-justify-center tw-gap-2">
